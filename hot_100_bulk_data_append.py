...
ef = open(filename, 'r') # r = read
currentline = ef.readline()
existingHeaders = currentline.rstrip('\n').split(',')

existingDateList = existingHeaders[2:len(existingHeaders)]

currentLine = ef.readline()
while currentLine:
    splitLine = currentLine.rstrip('\n').split('\",\"')

    existingSong = splitLine[0].replace('"', "")
    existingArtist = splitLine[1]

    songArtistKey = existingArtist.lower() + '-' + existingSong.lower()
    songList[songArtistKey] = {'song': existingSong, 'artist': existingArtist}

    for i in range(2, len(splitLine)):
        positionKey = existingSong.lower() + '-' + existingArtist.lower() + '-' + existingHeaders[i].rstrip('"').lower()
        positionList[positionKey] = splitLine[i].rstrip('"')
    currentLine = ef.readline()    

f = open(filename, 'w') # w = write

totalDateList = list(OrderedDict.fromkeys(existingDateList + dateList))

headers = 'Song,Artist,' + ','.join(map(str, totalDateList)) + '\n'
f.write(headers)
